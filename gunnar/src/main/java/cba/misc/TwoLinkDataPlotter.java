...
public class TwoLinkDataPlotter implements TabularFileHandler {
	...
	public void startRow(String[] row) {
		...
		final double sigma2 = Math.sqrt(Double.parseDouble(row[4]));
		final double corr = Double.parseDouble(row[5]) / sigma1 / sigma2;

		final String timeStr = this.df.format(time_s);

		this.exp1writer.println("(" + timeStr + "," + this.df.format(exp1) + ")");
		this.exp1lowerWriter.println("(" + timeStr + "," + this.df.format(exp1 - sigma1) + ")");
		this.exp1upperWriter.println("(" + timeStr + "," + this.df.format(exp1 + sigma1) + ")");

		this.exp2writer.println("(" + timeStr + "," + this.df.format(exp2) + ")");
		this.exp2lowerWriter.println("(" + timeStr + "," + this.df.format(exp2 - sigma2) + ")");
		this.exp2upperWriter.println("(" + timeStr + "," + this.df.format(exp2 + sigma2) + ")");

		this.corrWriter.println("(" + timeStr + "," + this.df.format(corr) + ")");
	}
