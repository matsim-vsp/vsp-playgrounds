			...
			public void install() {
				this.addControlerListenerBinding().toInstance(new BeforeMobsimListener() {
					...
					public void notifyBeforeMobsim(BeforeMobsimEvent event) {
						...
						Assert.assertEquals(rndOpeningTime_s, workParamsInControler.getOpeningTime(), 1e-8);
						Assert.assertEquals(rndClosingTime_s, workParamsInControler.getClosingTime(), 1e-8);
					}
				});
			}
		});
