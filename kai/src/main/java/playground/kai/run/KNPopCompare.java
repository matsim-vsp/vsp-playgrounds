...
import static org.matsim.contrib.analysis.kai.KNAnalysisEventsHandler.CERTAIN_LINKS_CNT;
import static org.matsim.contrib.analysis.kai.KNAnalysisEventsHandler.PAYMENTS;
import static org.matsim.contrib.analysis.kai.KNAnalysisEventsHandler.TRAV_TIME;
...
//				config.plans().setInputPersonAttributeFile(args[2]);
				throw new RuntimeException("include person attributes in population. no extra file needed.") ;
//			} else {
...
//				throw new RuntimeException("no person attributes") ;
		...
		for ( Person person1 : pop1.getPersons().values() ) {
				...
				{ // process money differences:
					Double payments1 = (Double) person1.getAttributes().getAttribute( PAYMENTS ) ;
					if ( payments1==null ) {
						payments1 = 0. ;
					}
					Double payments2 = (Double) person2.getAttributes().getAttribute( PAYMENTS ) ;
					if ( payments2==null ) {
						payments2 = 0. ;
					}
					final double deltaPayments = payments2 - payments1;

					person1.getAttributes().putAttribute( "deltaMoney", deltaPayments ) ;
					
					ActivityFacility fac = ff.createActivityFacility(Id.create(person1.getId().toString(), ActivityFacility.class), coord) ;
					fac.getAttributes().putAttribute(GridUtils.WEIGHT, deltaPayments ) ; 
					...
					Double certainLinksCnt = (Double) person1.getAttributes().getAttribute( CERTAIN_LINKS_CNT ) ;
				...
				{ // process travtime differences:
					Double item1 = (Double) person1.getAttributes().getAttribute( TRAV_TIME ) ;
					if ( item1==null ) {
						item1 = 0. ;
					}
					Double item2 = (Double) person2.getAttributes().getAttribute( TRAV_TIME ) ;
					if ( item2==null ) {
						item2 = 0. ;
					}
					final double deltaTtime = item2 - item1;

					person1.getAttributes().putAttribute( "deltaTtime", deltaTtime ) ;
					
					ActivityFacility fac = ff.createActivityFacility(Id.create(person1.getId().toString(), ActivityFacility.class), coord) ;
					fac.getAttributes().putAttribute(GridUtils.WEIGHT, deltaTtime ) ;
		...
		try ( BufferedWriter writer = IOUtils.getBufferedWriter("personscompare.txt") ) {
			...
			for ( Person person : pop1.getPersons().values() ) {
				if ( first ) {
					...
					for ( String key : person.getAttributes().getAsMap().keySet() ) { // design is such that this might vary person by person :-(
						writer.write( "\t" + key );
					}
					writer.newLine() ;
				}
				...
				// ---
				for ( Entry<String,Object> entry : person.getAttributes().getAsMap().entrySet() ) {
