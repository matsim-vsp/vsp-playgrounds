...
public class CongestionAvertingTravelDisutility
    ...
    private final double overFlowPenalty;

    public CongestionAvertingTravelDisutility(Config config) {
        this.config = config;
        discountFactor = 0.9;
        penaltyFactor = 2.0;
        overFlowPenalty = 2 * updatePeriod;
    }

    public CongestionAvertingTravelDisutility(Config config, double discountFactor, double penaltyFactor,
                                              double overFlowPenaltyFactor) {
        this.config = config;
        this.discountFactor = discountFactor;
        this.penaltyFactor = penaltyFactor;
        this.overFlowPenalty = overFlowPenaltyFactor * updatePeriod;
    }
    ...
    public double getLinkTravelDisutility(Link link, double time, Person person, Vehicle vehicle) {
        ...
        if (linksOccupationMap.containsKey(link.getId())) {
            ...
            double overFlowPenalty = overFlow / flowCapacityPerSec * this.overFlowPenalty;
