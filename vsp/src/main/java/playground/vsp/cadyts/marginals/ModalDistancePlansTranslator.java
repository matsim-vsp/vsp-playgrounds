...
public class ModalDistancePlansTranslator implements PlansTranslator<Id<DistanceBin>> {
	...
	public cadyts.demand.Plan<Id<DistanceBin>> getCadytsPlan(Plan plan) {
		...
		if (plan.getCustomAttributes().containsKey(BUILDER_KEY)) {
			return ((PlanBuilder<Id<DistanceBin>>) plan.getCustomAttributes().get(BUILDER_KEY)).getResult();
		}
		return null;
	}

	@SuppressWarnings("unchecked")
	void addTurn(Plan forPlan, Id<DistanceBin> turn, int iteration) {
		...
		PlanBuilder<Id<DistanceBin>> builder = (PlanBuilder<Id<DistanceBin>>) forPlan.getCustomAttributes().get(BUILDER_KEY);
