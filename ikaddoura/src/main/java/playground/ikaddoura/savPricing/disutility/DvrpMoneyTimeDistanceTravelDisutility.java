...
public class DvrpMoneyTimeDistanceTravelDisutility implements TravelDisutility {
	...
	public DvrpMoneyTimeDistanceTravelDisutility(
		...
		this.marginalUtilityOfTime_sec = (scenario.getConfig().planCalcScore().getModes().get(DefaultTaxiOptimizerProvider.TAXI_OPTIMIZER).getMarginalUtilityOfTraveling() +
				(-1. * scenario.getConfig().planCalcScore().getPerforming_utils_hr() )  ) / 3600.;
		
		this.marginalUtilityOfDistance_m = scenario.getConfig().planCalcScore().getModes().get(DefaultTaxiOptimizerProvider.TAXI_OPTIMIZER).getMarginalUtilityOfDistance() + 
				scenario.getConfig().planCalcScore().getModes().get(DefaultTaxiOptimizerProvider.TAXI_OPTIMIZER).getMonetaryDistanceRate() * marginalUtilityOfMoney;
		...
		if (marginalUtilityOfTime_sec >= 0. && marginalUtilityOfTime_sec >= 0.) {
			throw new RuntimeException("Check scoring parameters for " + DefaultTaxiOptimizerProvider.TAXI_OPTIMIZER + ".");
		}
