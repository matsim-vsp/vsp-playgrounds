...
public class RunBerlinDrtPricingScenarioB {
	private static final Logger log = Logger.getLogger(RunBerlinDrtPricingScenarioB.class);
	...
	public static void main(String[] args) {
		...
		if (args.length > 0) {		
			...
			configFileName = baseDirectory + "scenarios/berlin-v5.2-1pct/input/berlin-drt1-v5.2-1pct.config.xml";
		...
		RunBerlinDrtPricingScenarioB drtPricing = new RunBerlinDrtPricingScenarioB();
		drtPricing.run();
	}
	
	private void run() {
		RunBerlinDrtScenarioB berlin = new RunBerlinDrtScenarioB(configFileName, overridingConfigFileName, carRestrictedAreaShapeFile, drtServiceAreaShapeFile, transitStopCoordinatesSFile);
		...
		final SAVPricingConfigGroup optAVParamsDRT = ConfigUtils.addOrGetModule(config, SAVPricingConfigGroup.class);
		...
		// sav pricing
		controler.addOverridingModule(new SAVPricingModule(scenario, RunBerlinDrtScenarioB.modeToReplaceCarTripsInBrandenburg));
		...
		modes.add(RunBerlinDrtScenarioB.modeToReplaceCarTripsInBrandenburg);
