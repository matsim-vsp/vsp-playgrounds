 ...
 * @author gleich
 */
public class DrtPtTripEventHandler
		implements ActivityStartEventHandler, ActivityEndEventHandler, PersonDepartureEventHandler,
		PersonArrivalEventHandler, PersonEntersVehicleEventHandler, LinkEnterEventHandler,
		TeleportationArrivalEventHandler, AgentWaitingForPtEventHandler, DrtRequestSubmittedEventHandler,
	...
	public DrtPtTripEventHandler(Network network, TransitSchedule ptSchedule, Set<String> monitoredModes,
			Set<Id<Link>> monitoredStartAndEndLinks) {
	...
	public void handleEvent(LinkEnterEvent event) {
		if (monitoredVeh2toMonitoredDistance.containsKey(event.getVehicleId())) {
			monitoredVeh2toMonitoredDistance.put(event.getVehicleId(),
					monitoredVeh2toMonitoredDistance.get(event.getVehicleId()) + network.getLinks()
							.get(event.getLinkId())
							.getLength());
		}
	}
	...
	public void handleEvent(PersonDepartureEvent event) {
		...
		if (!monitoredModes.contains(event.getLegMode())) {
			...
			if (agent2CurrentLegStartLink.containsKey(event.getPersonId())) {
				throw new RuntimeException("agent "
						+ event.getPersonId()
						+ " has PersonDepartureEvent at time "
						+ event.getTime()
						+ " although the previous leg is not finished yet.");
			} else {
		...
		if (agent2CurrentTripStartLink.containsKey(event.getPersonId())) {
			if (agent2CurrentLegMode.get(event.getPersonId()).equals(event.getLegMode())) {
				...
				throw new RuntimeException("leg mode at PersonArrivalEvent different from leg mode saved at last "
						+ "PersonDepartureEvent for agent "
						+ event.getPersonId()
						+ " at time "
						+ event.getTime());
			}
