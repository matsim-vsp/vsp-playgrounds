 ...
 * (threshold) n= 250 veh*s   (sbraun Feb.18)
 * 
...
public class DgRoederGershensonSignalController implements SignalController {
	...
	//If in the distance there are more vehicles as physically possible, add the maximal critical number of vehicles to the counter
...
//	//This is just to demonstrate that the algorithm works
		...
		//fills approachingVehiclesGroupMap and counter (counter: Every second the number of vehicles in the monitored distance is added)
			carsApproachInGroup();
			updatecounter();
		//Find the group with the highest count given it has no outbound jam
