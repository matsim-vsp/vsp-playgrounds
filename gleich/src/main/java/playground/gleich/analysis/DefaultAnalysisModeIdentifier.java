...
/**
 * Copy from matsim-berlin repository OpenBerlinIntermodalPtDrtRouterAnalysisModeIdentifier
 *
 * @author gleich
 *
 */
public final class DefaultAnalysisModeIdentifier implements AnalysisMainModeIdentifier {
	...
	@Inject
	public DefaultAnalysisModeIdentifier() {
		...
		modeHierarchy.add( "bicycle" ); // in matsim-berlin TransportMode.bike is not registered as main mode, only "bicycle" ;
		modeHierarchy.add( TransportMode.ride ) ;
		modeHierarchy.add( TransportMode.car ) ;
		modeHierarchy.addAll(drtModes);
